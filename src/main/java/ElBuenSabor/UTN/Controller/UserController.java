...
import ElBuenSabor.UTN.Models.DTO.AssingRoleDTO;
...
public class UserController {
    ...
    public ResponseEntity<?> getUserById(@RequestBody UserDTO userDTO) {
        try {
            Usuario user = userBBDDService.findById(userDTO.getAuth0Id());
            if(user == null) {
    ...
    public ResponseEntity<?> modifyUser(@RequestBody UserDTO userDTO) {
        try {
            ...
            Usuario updatedUser = Usuario.builder()
                    .auth0Id(newUser.getId())
                    .nombre(newUser.getName())
                    .apellido(userDTO.getLastName()) // o usar .apellido("Default") si no hay campo
                    .email(newUser.getEmail())
                    .roles(rolesAsignados)
                    .build();
    ...
    public ResponseEntity<?> assignRoles(@RequestBody AssingRoleDTO request) {
        try {
            ...
            Usuario user = userBBDDService.findById(request.getId());
